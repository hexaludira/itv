...
if (isset($_GET['kode'])) {
  $sql_cek = "SELECT 
              ...
              FROM Purchase 
              INNER JOIN supplier ON Purchase.SupplierNo=Supplier.SupplierNo 
              WHERE Purchase.PurchaseNo='" . $_GET['kode'] . "' AND Supplier.IsDelete='0'";
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item active"><a class='h10'>IT Inventory Active menu - <b>Master Data>Purchase Order>Purchase Order Detail</b></a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                              ...
                              <td>
                                <?php 
                                   $sql2 = $koneksi->query("SELECT B.item_category_code as itemcat                                                                                                                     
                                                            ...
                                                            WHERE A.PurchaseNo='" . $_GET['kode'] . "' AND A.PurchaseItemNo='" . $data['PurchaseItemNo'] . "' AND A.Matnr='" . $data['Matnr'] . "' AND A.Werks='" . $data['Werks'] . "' AND A.isdelete='1'");
                              ...
                              <td>
                                <?php 
                                    $sql2 = $koneksi->query("SELECT IFNULL( (SELECT A.price
                                                              FROM itv_masterdata_po_price as A
                                                              INNER JOIN itv_masterdata_currency as B ON A.id_currency = B.id_currency                                                   
                                                              WHERE A.PurchaseNo='" . $_GET['kode'] . "' AND A.PurchaseItemNo='" . $data['PurchaseItemNo'] . "' AND A.Matnr='" . $data['Matnr'] . "' AND A.Werks='" . $data['Werks'] . "' AND A.isdelete='1'),'1')");                                   
                                    
                                    while ($data2= $sql2->fetch_row()) {   
                                      $row_cnt = $data2[0];                                                                                                            
                                      if ($row_cnt == '1') 
                                      { ?>
                                        <button class="btn btn-primary btn-sm btn_edit" name="<?php echo $_GET['kode'] ?>" id="<?php echo $data['PurchaseItemNo']; ?>"><i class="fas fa-plus"></i>
                                          Add Price & Link Document
                                      </button>
                                      <?php
                                      }
                                      else { ?> 
                                        <button class="btn btn-primary btn-sm btn_edit" name="<?php echo $_GET['kode'] ?>" id="<?php echo $data['PurchaseItemNo']; ?>" disabled><i class="fas fa-plus"></i>
                                          Add Price & Link Document
                                        </button>
                                      <?php
                                      }
                      ...
                      <div class="col-md-12">
                        ...
                        <div class="form-group">
                          ...
                          <div class="form-group">   
                            ...
                            <input type="text" class="form-control" id="PurchaseItemNo" name="PurchaseItemNo" disabled>                         
                          </div>      
                          <div class="form-group">
                          ...
                          <div class="form-group">
                           <label><h5>KEK's Document</h5></label>
                              <select class="form-control custom-select" id="id_po_dokumen" name="id_po_dokumen">
                                <?php 
                                  $documentisi = mysqli_query($koneksi, "select * from itv_masterdata_po_dokumen as A where A.PurchaseNo='" . $_GET['kode'] . "'"); 
                                  while($row = mysqli_fetch_array($documentisi)){?>
                                      <?php                                  
                                        echo "<option value=$row[id_po_dokumen]>$row[nomorDokumen] - $row[tanggalDokumen]</option>";
                                      ?>
                                <?php } ?>
                              </select>
                          </div> 
...
$(document).ready(function(){
        ...
        $('#example2').on('click','.btn_edit', function(e){
            e.preventDefault();
              var PurchaseItemNo = $(this).attr('id');
              var PurchaseNo = $(this).attr('name');
              //$("#modal_add_price").modal("show");
              $.ajax({
                url : "itv/administrator/masterdata/po/po_add_price_controller.php",
                type : 'POST',
                data : {PurchaseItemNo_Edit : PurchaseItemNo, PurchaseNo : PurchaseNo},
                dataType : 'json',
                success : function(response){
                  //console.log(response);
                  $("#modal_add_price").modal("show");
                  $('#PurchaseItemNo').val(response.PurchaseItemNo);
      ...
      $('#btn_add_price').on('click', function(e){
        ...
        $.ajax({
          ...
          data : {Werks : Werks, 
                  ...
                  PurchaseItemNo_save : PurchaseItemNo, 
