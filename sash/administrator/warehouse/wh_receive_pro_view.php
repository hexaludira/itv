        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item active"><a class='h10'>Active menu/<b>WAREHOUSE/Warehouse Order/Warehouse Order Detail/Receive</b></a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                          ...
                          <table id="example2" class="table table-bordered table-striped" cellspacing="0">
                            ...
                            <tr>
                              ...
                              <td>
                                <?php $status = $data['status'];?>
                                <?php if($status == "10"){?>
                                  <button class="btn btn-info btn-sm btn_edit" id="<?php echo $data['id_wh_detail']; ?>" name="<?php echo $data['id_wh_detail_root']; ?>"><i class="fas fa-hand-holding-medical"></i></button>
                                  <button class="btn btn-default btn-sm btn_add" id="<?php echo $data['id_wh_detail']; ?>"><i class="fas fa-plus"></i></button>
                                  <button class="btn btn-default btn-sm btn_delete" id="<?php echo $data['id_wh_detail']; ?>" name="<?php echo $data['id_wh_detail_root']; ?>"><i class="fas fa-trash-can"></i>Delete</button>
                                <?php }elseif($status == "70"){?>
                                  <button class="btn btn-info btn-sm btn_edit" id="<?php echo $data['id_wh_detail']; ?>" name="<?php echo $data['id_wh_detail_root']; ?>" disabled><i class="fas fa-hand-holding-medical"></i></button>
...
<!------------------- Modal Edit/Receive WO detail ---------------------------------->
<div class="modal fade" id="wh_detail_edit_modal">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title">Receive Order Item</h4>
        ...
        <div class="row">
            ...
            <button type="button" class="btn btn-success" id="btn_po_detail_receive">Receive</button>
				</div>
			</div>
		</div>
	</div>
...
<div class="modal fade" id="add_confirmation_modal" tabindex="-1">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title">Add More Receive</h4>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
          <h4>Are you sure you want to add another receive?</h4>
				</div>
				<div class="modal-footer">
...
<script type="text/javascript">
  ...
  $($.fn.dataTable.tables(true)).DataTable()
      ...
      $('#example2').on('click','.btn_edit', function(e){
        ...
        $.ajax({
          url : "sash/administrator/warehouse/wh_receive_pro_controller.php",
          type : 'POST',
				...
				$.ajax({
                  ...
                  batch_update : batch
					...
					success : function(response){
						console.log(response);
            if(response == "1"){
              Swal.fire("Data received succesfully",'','success');
            } else {
              Swal.fire("Receive data failed",'','error');
            }
